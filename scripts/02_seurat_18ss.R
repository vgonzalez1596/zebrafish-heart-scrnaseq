...
# Seurat analysis, standard workflow, 18 ss dataset
...
# 18 ss UMAP with all clusters
heart18s_final_clustersnamed <- heart18s_final
new_cluster_names <- c("Mixed Cell Types", "Pharyngeal Arch", "Cardiomyocyte", "Brain", "Neural Crest", "Epidermis", "Somite", "Hatching Gland", "Vasculature", "Blood Cells")
...
DimPlot(heart18s_final_clustersnamed, reduction = "umap", pt.size = .5)
...
# 18 ss UMAP with the cardiac cluster in green, all other clusters in grey
DimPlot(heart18s_final, reduction = "umap", cols = c("darkgrey", "darkgrey", "#0a9f0a", "darkgrey", "darkgrey", "darkgrey", "darkgrey", "darkgrey", "darkgrey", "darkgrey")) + NoLegend()
p_umap_cardiac <- DimPlot(heart18s_final, reduction = "umap", cols = c("darkgrey", "darkgrey", "#0a9f0a", "darkgrey", "darkgrey", "darkgrey", "darkgrey", "darkgrey", "darkgrey", "darkgrey")) + NoLegend()
ggsave(
...
write.csv(heart18s_final.markers, file = file.path(results_dir, "cluster_markers_heart18ss.csv"), row.names = TRUE)
top10 <- heart18s_final.markers %>%
  group_by(cluster) %>%
  top_n(n = 10, wt = avg_log2FC)
DoHeatmap(heart18s_final, features = top10$gene) + NoLegend() + theme(axis.text.x = element_text(angle = 0, hjust = 0.5))
p_heatmap <- DoHeatmap(heart18s_final, features = top10$gene) + NoLegend() + theme(axis.text.x = element_text(angle = 0, hjust = 0.5))
ggsave(
...
heart18s_final_reordered <- SetIdent(heart18s_final, value = factor(Idents(heart18s_final), levels = c("2", "0", "1", "3", "4", "5", "6", "7", "8", "9")))
new_cluster_names <- c("2" = "H", "0" = "MCT", "1" = "PA", "3" = "CNS", "4" = "NC", "5" = "EP", "6" = "S", "7" = "HG", "8" = "V", "9" = "BC")
heart18s_final_reordered <- RenameIdents(heart18s_final_reordered, new_cluster_names)
...
left_sided_genes_positive <- WhichCells(heart18s_final_cluster2only, expression = pitx2 > .3872 | lft2 > .9976 | lft1 > .3455 | ndr2 > .2029)
heart18s_final_cluster2only <- SetIdent(heart18s_final_cluster2only, cells = left_sided_genes_positive, value = 'Lpositive')
left_sided_genes_negative <- WhichCells(heart18s_final_cluster2only, expression = lft2 == 0 & pitx2 == 0 & lft1 == 0 & ndr2 == 0)
heart18s_final_cluster2only <- SetIdent(heart18s_final_cluster2only, cells = left_sided_genes_negative, value = 'Rnegative')
# Save object with left and right identities
saveRDS(heart18s_final_cluster2only, file = file.path(results_dir, "heart18s_cluster2_LR.rds"))

# LR differential expression
heart18s_LR_DEanalysis.markers <- FindMarkers(heart18s_final_cluster2only, ident.1 = "Lpositive", ident.2 = "Rnegative")
heart18s_LR_DEanalysis.markersFILTERED <- subset(heart18s_LR_DEanalysis.markers, p_val_adj < 0.05 & abs(avg_log2FC) > 0.25)
write.csv(heart18s_LR_DEanalysis.markersFILTERED, file = file.path(results_dir, "LR_DEanalysis_heart18ss_filtered.csv"), row.names = TRUE)

#Observing Nodal BMP, and FGF signaling in 18 ss dataset across all clusters via AUCell. 
...
new_cluster_names <- c("2" = "H", "0" = "MCT", "1" = "PA", "3" = "CNS", "4" = "NC", "5" = "EP", "6" = "S", "7" = "HG", "8" = "V", "9" = "BC")
heart18s_final_reordered <- RenameIdents(heart18s_final_reordered, new_cluster_names)
p_auc <- VlnPlot(heart18s_final_reordered, features = names(geneSets), cols = c("#0a9f0a", "darkgrey", "darkgrey", "darkgrey", "darkgrey", "darkgrey", "darkgrey", "darkgrey", "darkgrey", "darkgrey")) +  
  theme(legend.position = "none", plot.title = element_blank(), axis.text.x = element_text(angle = 0, hjust = 0.5, size = 10), axis.title.y = element_text(size = 16)) +  
  labs(y = "Nodal signaling at 18 ss") + xlab("Cluster")
ggsave(
  filename = file.path(figures_dir, "18ss_AUCell_Nodal_by_cluster.png"),
  plot = p_auc,
  width = 7, height = 5, dpi = 300
)
# Save object w/ AUCell metadata
